...
class WrappedVGG(nn.Module):
    def __init__(self, net, custom_num_classes=None, img_size=224):
        ...
        if custom_num_classes is not None:
            ...
            self.net.classifier[-1] = nn.Linear(in_dim, custom_num_classes)
...
def wrapped_vgg11(custom_num_classes=None):
    net = vgg11(pretrained=True)
    return WrappedVGG(net, custom_num_classes)

def wrapped_vgg11_bn(custom_num_classes=None):
    net = vgg11_bn(pretrained=True)
    return WrappedVGG(net, custom_num_classes)

def wrapped_vgg13(custom_num_classes=None):
    net = vgg13(pretrained=True)
    return WrappedVGG(net, custom_num_classes)

def wrapped_vgg13_bn(custom_num_classes=None):
    net = vgg13_bn(pretrained=True)
    return WrappedVGG(net, custom_num_classes)

def wrapped_vgg16(custom_num_classes=None):
    net = vgg16(pretrained=True)
    return WrappedVGG(net, custom_num_classes)

def wrapped_vgg16_bn(custom_num_classes=None):
    net = vgg16_bn(pretrained=True)
    return WrappedVGG(net, custom_num_classes)

def wrapped_vgg19(custom_num_classes=None):
    net = vgg11(pretrained=True)
    return WrappedVGG(net, custom_num_classes)

def wrapped_vgg19_bn(custom_num_classes=None):
    net = vgg11_bn(pretrained=True)
    return WrappedVGG(net, custom_num_classes)

'''Below directly copied from the link
   https://github.com/pytorch/vision/blob/973db14523d338fa4e772574fe4d49763fb25245/torchvision/models/vgg.py
'''
