...
# preprocess ResNet50
class gan_proj_ft_net(ft_net):

    def __init__(self, *args, use_penultimate=True, flip=True, **kwargs):
        super(gan_proj_ft_net, self).__init__(*args, circle=use_penultimate,
                                              **kwargs)

        self.use_penultimate = use_penultimate
        self.flip = flip
        self.mean = torch.tensor([0.485, 0.456, 0.406], requires_grad=False)
        self.std = torch.tensor([0.229, 0.224, 0.225], requires_grad=False)

    def _apply(self, fn):
        super(gan_proj_ft_net, self)._apply(fn)
    ...
    def forward(self, x):
        ...
        # scale from [-1, 1] to normalize range
        x = (x + 1.) / 2. # [0, 1]
        x = (x - self.mean.view(-1, 1, 1)) / self.std.view(-1, 1, 1)

        x_flip = self._fliplr(x)

        if self.use_penultimate:
            x = super(gan_proj_ft_net, self).forward(x, feat_extract=True)
            # _, x = super(gan_proj_ft_net, self).forward(x)
        else:
            x = super(gan_proj_ft_net, self).forward(x)

        if not self.flip:
            return x

        if self.use_penultimate:
            x_flip = super(gan_proj_ft_net, self).forward(x_flip, feat_extract=True)
            # _, x_flip = super(gan_proj_ft_net, self).forward(x_flip)
        else:
            x_flip = super(gan_proj_ft_net, self).forward(x_flip)
